...
import (
	...
	"sort"
	"strings"
	_ "xutil/cmd/springCloud分布式框架/cloud/resources"
)
...
func Run(target, name, path string) {
	if path != "/" {
		root(target, name)
	}
...
func GetFiles(path string) []string {
	...
	CheckError(err)
	var files []string
	err = fs.Walk(sss, path, func(path string, fi os.FileInfo, err error) error {
		CheckError(err)
